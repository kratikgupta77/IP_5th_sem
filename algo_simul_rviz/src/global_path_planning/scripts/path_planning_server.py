...
from algorithms.wt_astar import wt_astar
...
def make_plan(req):
  ...
  path, previous_plan_variables = prm(start_index, goal_index, width, height, costmap, resolution, origin, viz, previous_plan_variables)
  ...
  if not path:
    ...
    #rospy.loginfo(req.start.x)
    rospy.loginfo(goal_index/width)
    print("\n")
    rospy.loginfo(goal_index/height)
    #print(req.goal.x)
